...
# Robust column cleaning (lowercase, remove whitespace, replace spaces with _)
...
groups = [group['Time'].values for name, group in data.groupby('treatment')]
stat, p = stats.levene(*groups)
print(f"Levene's test: W={stat:.3f}, p={p:.4f}")
model = ols('Time ~ C(temp) * C(humidity)', data=data).fit()
...
sns.boxplot(x='temp', y='Time', hue='humidity', data=data)
plt.title('Time by Temperature and Humidity')
